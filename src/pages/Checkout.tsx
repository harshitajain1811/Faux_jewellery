  ...
  useEffect(() => {
    async function prefillUserProfile() {
      ...
      try {
        ...
        if (profile) {
          setInitialFormData({
            email: props.user.email || '', // Retaining the email string from Supabase Auth mapping
...
function CheckoutFormInterior({ cartItems, user, onOrderPlacedSuccess, navigateToView, structuralPreFill }: InteriorProps) {
  ...
  if (cartItems.length === 0 && !guestSuccessMessage) return null;
          ...
          {checkoutStep === 'shipping' && (
            <form onSubmit={handleProceedToPayment} className="space-y-5">
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="space-y-1">
                  <label className="text-[9px] font-sans tracking-wider uppercase text-stone-400">Postal Index (6 Digits)</label>
              ...
              <button type="submit" className="w-full mt-4 bg-stone-950 text-white font-sans text-xs tracking-widest uppercase py-4 border border-stone-950 hover:bg-transparent hover:text-stone-950 transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer">
